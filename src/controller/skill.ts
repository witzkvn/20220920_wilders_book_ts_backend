...
<<<<<<< HEAD
  create: async (req, res) => {
    try {
      const createdSkill = await dataSource.getRepository(Skill).save(req.body);

      res
        .status(201)
        .send({ message: "Created skill", newWilder: createdSkill });
    } catch (error) {
  ...
  updateById: async (req, res) => {
    try {
      ...
      if (isNaN(skillId)) {
        res
          .status(400)
          .send({ message: "No valid skill ID specified in the route" });
        return;
      }
      ...
      res
        .status(500)
        .send({ message: "Error while updating the requested skill" });
    }
  },
  deleteById: async (req, res) => {
    try {
      ...
      res
        .status(500)
        .send({ message: "Error while deleting the requested skill" });
    }
  },
=======
    create: async (req, res) => {
    ...
    deleteById: async (req, res) => {
        try {
            ...
            if (deleteRes.affected === 0) {
                res.status(400).send({
                    message:
                        "No delete could be effectued. Please check the skill id.",
                });
            } else {
                res.status(200).send({ message: "Skill deleted successfully" });
            }
